	...
	if command -v patch >/dev/null 2>&1; then
		for patch_file in "${patch_files[@]}"; do
			if [ ! -f "${patch_file}" ]; then
				echo "[P1102GT] Missing patch: ${patch_file}"
			elif (
				cd "${workspace_root}" &&
				...
				) && echo "[P1102GT] Applied patch: $(basename "${patch_file}")"
			else
				echo "[P1102GT] Patch already applied or not applicable: $(basename "${patch_file}")"
			fi
		done
	fi
